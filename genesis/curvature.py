...
def plot_edges_with_curvature(mask, min_contour_length, window_size_ratio):
    ...
    # Debugging: Print the length of curvature_values and edge_pixels
    print(f"Number of edge pixels: {len(edge_pixels)}")
    print(f"Number of curvature values: {len(curvature_values)}")

    if len(curvature_values) == 0:
        print("No curvature values computed. Check the mask and contour detection.")
        return
...
mask = np.array(cropped_map.data)
