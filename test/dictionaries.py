...
centroids_1 = np.array([[3.1, 4.1], [1.1, 2.1], [5.1, 6.1]])
...
for frame_index, ID_and_centroid in enumerate(ID_centroids):
    ...
    if frame_index == 0:
        ...
        matches = trajectories(centroids_old, centroids_new)
        ...
        for old, new, dist in matches:
            ...
            trajectory = next((traj for traj in trajectories_list if traj["index"] == old and traj["id"] not in matched_trajectories), None)
            if trajectory:
                ...
                matched_trajectories.add(trajectory["id"])
